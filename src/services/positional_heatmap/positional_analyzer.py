...
class PositionalAnalyzer:
    ...
    def get_detailed_evaluation(self, board: chess.Board, 
        ...
        Returns:
            Dictionary with detailed evaluation info per piece.
        """
        if perspective is None:
            perspective = board.turn
        ...
        # Get final aggregated scores
        aggregated = self.aggregator.aggregate([scores for _, scores in rule_results], 
                                                [rule for rule, _ in rule_results])
